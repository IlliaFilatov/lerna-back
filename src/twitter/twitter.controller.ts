...
let accessToken = '1369638430259482625-BrysOoJA6n4zDPhr8V4YWJ6l4J30F7';
let accessSecret = 'QD4yMaXVHpwG3JJyRLCIcddpBKpNq1HclN9PeJFZZiytn';
...
export class TwitterController {
  ...
  async confirm(@Body() body: { code: string }) {
    ...
    // });
    const data = await twitterClient.login(code);
  ...
  async upload(
    ...
    const data = await readWriteClient.v1.uploadMedia(file.buffer, {
      mimeType: file.mimetype,
    });
    console.log('data: ', data);
    const result = await readWriteClient.v2.tweet(uploadMediaDto.message, {
      media: {
        media_ids: [data],
      },
    });
